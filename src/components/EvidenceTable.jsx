...
export default function EvidenceTable() {
  ...
  const EvidenceLink = ({ value, label, type }) => {
    ...
    if (value === "Không yêu cầu") {
      return <span className="px-2 py-1 text-xs font-bold bg-academic-cream text-academic-muted rounded">---</span>;
    }
    return (
      <a 
        href={value} 
  ...
  const StatusBadge = ({ report, img, drive }) => {
    ...
    return (
      <span className="inline-flex items-center gap-1 px-2 py-1 bg-academic-cream text-academic-blue text-xs font-bold rounded border border-academic-border">
    ...
    <div className="max-w-6xl mx-auto animate-fade-in pb-10">
      ...
      {previewData.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 md:p-10 animate-fade-in print:hidden">
          <div className="bg-academic-card w-full max-w-5xl h-full max-h-[90vh] rounded-2xl flex flex-col overflow-hidden shadow-2xl">
      ...
      <div className="bg-academic-card rounded-2xl shadow-sm border border-academic-border overflow-hidden">
        <div className="bg-academic-cream border-b border-academic-border p-4 flex items-start gap-3">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[850px]">
            <thead>
              <tr className="bg-academic-cream border-b border-academic-border text-academic-muted text-sm uppercase tracking-wider">
            ...
            <tbody className="divide-y divide-slate-100">
              {projects.map((project) => (
                <tr key={project.id} className="hover:bg-academic-cream transition-colors">
                  <td className="p-4">
